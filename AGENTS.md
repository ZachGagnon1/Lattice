...
`packages/lattice/src/` splits into three top-level parts. `domain/` holds the blocks, the compile logic, the constants, and the typings — this is framework-agnostic core logic. `adapters/` holds `ui/` and `panels/` — the React components and hooks. `shared/` holds the utils and typings that both sides use.
...
IBlockData<Attr, Data> = {
  type: string              // e.g. BasicType.IMAGE
...
`BasicType` is the one enum of block types (page, section, column, text, image, button, table, condition, for-loop, etc.), defined in `packages/lattice/src/domain/constants.ts`. `AdvancedType` is a deprecated alias for `BasicType` — `export const AdvancedType = BasicType`. An older type such as `advanced_text` no longer exists as its own block type. `BlockManager.toBasicType()` strips a legacy `advanced_` prefix only, to keep old saved templates working.

Each block has an `IBlock<T>` definition with `create()` (factory), `validParentType[]`, and `render()`. All blocks live in one flat directory: `packages/lattice/src/domain/blocks/definitions/`.
...
The editor uses **react-hook-form** for the form state. The fields are `subject`, `subTitle`, and `content`. The `content` field holds the root `IPage` block. The `EmailEditorProvider` calls `useForm` and wraps the tree in `FormProvider`. A new `data` prop goes through `reset()`. The `EditorFormProvider` (`packages/lattice/src/adapters/ui/Provider/EditorFormProvider`) holds the only `useWatch` subscription for the whole form.

- react-hook-form sends a new deep clone of the values on each change. The `replaceEqualDeep` function in `packages/lattice/src/shared/utils/formValues.ts` keeps the identity of each unchanged block. The memoized components compare the blocks by identity.
...
The table matches `OPERATOR_HELPER_NAMES` in `packages/lattice/src/domain/compile/handlebars/types.ts`.
...
`packages/lattice/src/shared/utils/variableSchema.ts` holds this logic. zod is a
...
`packages/lattice/src/shared/typings/variableData.ts` holds the path types:
...
- **Custom blocks** are created via `createBlock()` and registered via `BlockManager.registerBlocks()`. `createCustomBlock()` still works, as a legacy alias for `createBlock` (see `packages/lattice/src/domain/blocks/createCustomBlock.ts`).
