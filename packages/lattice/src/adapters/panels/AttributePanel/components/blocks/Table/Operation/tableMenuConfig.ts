...
interface IMenuItem {
  text: string;
  icon?: string;
  handler: (this: ITableOperationMenu, color: string) => void;
}

// The menu computes the cell positions again before the next operation.
export const newCell = (): IOperationData => ({
  content: "-",
  top: 0,
  bottom: 0,
  left: 0,
...
const MENU_CONFIG = {
  insertColumnRight: {
    ...
    handler() {
      const right = this.tableIndexBoundary.right;
      this.tableData.forEach((tr) => {
        if (right === this.maxTdCount - 1) {
          tr.push(newCell());
          return;
        }
        if (tr.length === 0) return tr.push(newCell());
        for (let index = 0; index < tr.length; index++) {
          const tdLeft = tr[index].left || 0;
          const tdRight = tr[index].right || 0;
          if (tdRight === right) {
            tr.splice(index + 1, 0, newCell());
            break;
          }
  ...
  insertColumnLeft: {
    ...
    handler() {
      const left = this.tableIndexBoundary.left;
      this.tableData.forEach((tr) => {
        if (left === 0) {
          tr.unshift(newCell());
          return;
  ...
  insertRowUp: {
    ...
    handler() {
      const top = this.tableIndexBoundary.top;
      let maxTdCount = this.maxTdCount;
      if (this.tableData[top].length < maxTdCount) {
        for (let index = top - 1; index > -1; index--) {
          const tr = this.tableData[index];
          tr.forEach((td) => {
  ...
  insertRowDown: {
    ...
    handler() {
      let addCount = this.maxTdCount;
      const bottom = this.tableIndexBoundary.bottom;
      if (this.tableData[bottom].length < this.maxTdCount) {
        for (let index = bottom - 1; index > -1; index--) {
          const tr = this.tableData[index];
          if (tr.length === this.maxTdCount) break;
          tr.forEach((td) => {
      ...
      this.tableData[bottom].forEach((e) => {
        if (e.rowSpan && e.rowSpan > 1) {
          e.rowSpan += 1;
          addCount -= e.colSpan || 1;
        }
      });
      this.addRow(bottom + 1, addCount);
    },
  },
  mergeCells: {
    ...
    handler() {
      ...
      const leftTopItem = this.tableData[top].find(
        (e) => e.left === left,
      ) as IOperationData;
      leftTopItem.rowSpan = bottom - top + 1;
      leftTopItem.colSpan = right - left + 1;
      this.tableData.forEach((tr, trIndex) => {
  ...
  deleteColumn: {
    ...
    icon: `<svg width="20" height="20" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"><path fill="#595959" d="M925.996 99.038c-25.47-25.6-60.121-39.822-96.323-39.822H194.198c-75.12 0.13-136.016 61.026-136.145 136.146v635.345c0 36.073 14.351 70.723 39.822 96.323 25.6 25.73 60.25 40.081 96.323 40.081h635.475c36.072 0 70.723-14.351 96.323-39.951 25.6-25.6 39.951-60.122 39.951-96.324V195.362c0-36.073-14.351-70.724-39.951-96.324z m-365.77 494.287L512 545.228l-48.226 48.097-32.194-31.935 48.355-48.226-48.226-48.097 32.194-32.194L512 480.97l48.097-48.097 32.194 32.194-48.097 48.097 48.226 48.226-32.194 31.935zM103.434 195.362c0-24.049 9.568-47.192 26.635-64.13 17.066-17.066 40.08-26.634 64.129-26.634h136.145v226.91H103.434V195.361z m0 181.656h226.91V649.31h-226.91V377.02z m90.764 544.84c-24.049 0-47.192-9.567-64.13-26.634-17.066-17.066-26.634-40.08-26.634-64.258V694.69h226.91v227.168H194.197z m726.238-90.763c0 24.048-9.438 47.192-26.505 64.259-17.066 17.066-40.21 26.634-64.258 26.505H693.527V694.69h226.91v136.404z m0-181.786H693.527V377.02h226.91v272.29zM693.527 331.507V104.598h136.146c24.048 0 47.192 9.438 64.258 26.505 17.067 17.067 26.635 40.21 26.505 64.259v136.145H693.527z"/></svg>`,
    handler() {
      const { left, right } = this.tableIndexBoundary;
      this.tableData.forEach((tr) => {
  ...
  deleteRow: {
    ...
    handler() {
      const { top, bottom } = this.tableIndexBoundary;
      ...
      for (let index = top; index <= bottom; index++) {
        ...
        tr.forEach((td) => {
          ...
          if (rowSpan - 1 + top > bottom) {
            ...
            const nextRow = this.tableData[bottom + 1];
            if (nextRow) {
              const idx = Array.from({ length: this.maxTdCount }).findIndex(
      ...
      this.tableData.splice(this.tableIndexBoundary.top, deleteCount);
      this.changeTableData?.(this.tableData);
    },
  },
  setCellBg: {
    ...
    handler(color: string) {
      const { top, bottom, left, right } = this.tableIndexBoundary;
      this.tableData.forEach((tr) => {
        for (let index = 0; index < tr.length; index++) {
          const td = tr[index];
...
} satisfies Record<string, IMenuItem>;
